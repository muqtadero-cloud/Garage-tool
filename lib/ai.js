...
- DEFAULT to "Flat price" when ambiguous. Only choose "Unit price" or "Tier*" when you can QUOTE explicit non-time per-unit/usage language (e.g., "$5 per user", "$0.10 per API call").
- DO NOT treat time units as usage. Phrases like "$5,000 per month" indicate a flat recurring fee (monthly frequency), NOT unit pricing.
- Never populate event_to_track unless the contract explicitly defines the tracked usage event.
...
- "Unit price": Price varies by quantity used (e.g., "$10 per user", "$0.50 per API call"). Requires explicit per-unit language AND a non-time "unit". If the only unit is a time unit (month, year, week, day) then it is NOT unit pricing — it is a flat recurring price with frequency.
